...
export const mergeSections: CopySection[] = [
  ...
  },
  {
    ...
    p: [
      "Extract pages or cut a report into chapters on the [Split tab](/split). Range mode builds one PDF of the pages you listed; ‘one PDF per page’ writes a ZIP. After a split you can return here and merge a different subset.",
      "If Gmail or a portal refuses the merged file for size, open [Compress](/compress). Expect savings on digital exports (Word, invoices, slides). Do not expect a 40 MB photograph of paper to collapse — Folio does not recompress those images. Guides: [how to split a PDF in the browser](/how-to-split), and [how to compress without uploading](/how-to-compress).",
      "Gmail still refuses many packets around 25 MB — read the [email size guide](/email). A photograph of paper will not shrink here — read [why scans stay heavy](/scan). Step-by-step merge: [how to merge PDFs](/how-to-merge). Chat sends are a different cap: [WhatsApp](/whatsapp).",
    ],
  },
];
...
export const splitSections: CopySection[] = [
  ...
  },
  {
    ...
    p: [
      ...
      "A scan that is too heavy for mail is often a split job, not a compress job — read [why scans stay heavy](/scan). Attachment caps: [email size](/email). Chat: [WhatsApp](/whatsapp). After you have the pages you need, [merge them on the homepage](/).",
    ],
  },
];
...
export const compressSections: CopySection[] = [
  ...
  },
  {
    ...
    p: [
      ...
      "[Gmail and Outlook caps](/email). If before and after look alike, the file is probably a scan — [why scans stay heavy](/scan). Chat sends fail earlier: [WhatsApp](/whatsapp). Compress does not merge files; that is the [homepage](/).",
    ],
  },
];
...
export const useCaseSections: CopySection[] = [
  ...
  },
  {
    ...
    p: [
      "Chat apps are harsher than Gmail. A 12 MB slide deck can fail on mobile data. A 4 MB scan of handwritten notes can fail because the other person’s client is old. Folio does not send into WhatsApp. You compress or split here, download, then attach in the chat app yourself. Full walkthrough: [send a PDF in WhatsApp](/whatsapp).",
      "If you can select text in the deck, open [Compress](/compress), Balanced first. Read before/after. If it barely moved, the deck is full of photos of slides — that is a scan-like file even if it came from PowerPoint with ‘save as PDF’ after photographing a whiteboard.",
      "If they only asked for two pages of notes, [Split](/split). Range mode, one PDF. Do not merge extra catalogues ‘so they have everything’ and then fail the send. Chat caps punish generosity.",
